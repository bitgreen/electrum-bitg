...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-BITG.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-bitg-macos
	cd /tmp/electrum-bitg-macos
...
rm -rf /tmp/electrum-bitg-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-bitg-macos/image/
cp -r $1 /tmp/electrum-bitg-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-BITG_uncompressed.dmg \
    /tmp/electrum-bitg-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-BITG_uncompressed.dmg electrum-bitg-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-BITG_uncompressed.dmg

echo "Done."
sha256sum electrum-bitg-$VERSION.dmg
