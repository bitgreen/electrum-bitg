...
2. Build image
    ...
    ```
    $ sudo docker build -t electrum-bitg-wine-builder-img contrib/build-wine
    ```
...
3. Build Windows binaries
    ...
    $ FRESH_CLONE=contrib/build-wine/fresh_clone && \
        ...
        mkdir -p $FRESH_CLONE && \
        cd $FRESH_CLONE  && \
        git clone https://github.com/bitgreen/electrum-bitg.git && \
        cd electrum-bitg
    ```
    ...
    $ git checkout $REV
    $ sudo docker run -it \
        --name electrum-bitg-wine-builder-cont \
        -v $PWD:/opt/wine64/drive_c/electrum \
        --rm \
        --workdir /opt/wine64/drive_c/electrum/contrib/build-wine \
        electrum-bitg-wine-builder-img \
        ./build.sh
    ```
...
Electrum-BITG Windows builds are signed with a Microsoft Authenticode™ code signing
certificate in addition to the GPG-based signatures.

The advantage of using Authenticode is that Electrum-BITG users won't receive a
