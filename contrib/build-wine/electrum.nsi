...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-BITG"
  !define PRODUCT_WEB_SITE "https://github.com/bitgreen/electrum-bitg"
  !define PRODUCT_PUBLISHER "BitGreen Developers"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-bitg-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bitg-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-bitg-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-bitg-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-bitg-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-bitg-${PRODUCT_VERSION}.exe" 0


  ;Links bitgreen: URI's to Electrum-BITG
  WriteRegStr HKCU "Software\Classes\bitgreen" "" "URL:bitgreen Protocol"
  WriteRegStr HKCU "Software\Classes\bitgreen" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitgreen" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitgreen\shell\open\command" "" "$\"$INSTDIR\electrum-bitg-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\bitgreen"
